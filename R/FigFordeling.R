...
#' @export
#'
FigFordeling <- function(
  AggVerdier, tittel = 'mangler tittel', hovedgrTxt = '', smltxt = '',
  N, Nfig = 0, retn = 'H', subtxt = '', utvalgTxt = '', grtxt, grtxt2 = '',
  medSml = 0, antDes = 1, fargepalett = 'BlaaOff', outfile = ''
) {

  if (class(Nfig) != 'numeric') {
    Nfig <- N
  }
  ...
  if ((N$Hoved < 5) || (sum(N$Hoved + N$Rest) < 11)) {
    ...
    if (retn == 'H') {
      ...
      xmax <- max(c(AggVerdier$Hoved, AggVerdier$Rest), na.rm = TRUE) * 1.2
      xmax <- min(xmax, 100)
      ymin <- 0.3 # Fordi avstand til x-aksen av en eller annen grunn øker når antall sykehus øker
      ymax <- 0.4 + 1.25 * length(AggVerdier$Hoved)

      pos <- rev(barplot(rev(as.numeric(AggVerdier$Hoved)), xlim = c(0, xmax), ylim = c(ymin, ymax),
                         xlab = "Andel pasienter (%)", horiz = TRUE, border = NA, col = fargeHoved))
      ...
      if (medSml == 1) { #Legge på prikker for sammenlikning
        points(as.numeric(AggVerdier$Rest), pos, col = fargeRest,  cex = 2, pch = 18)
      }
    ...
    if (retn == 'V') {
      ...
      ymax <- min(max(c(AggVerdier$Hoved, AggVerdier$Rest), na.rm = TRUE) * 1.25, 115)
      ...
      if (medSml == 1) {
        ...
        points(pos, as.numeric(AggVerdier$Rest), col = fargeRest,  cex = 2, pch = 18)
